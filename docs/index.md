...
[Python](https://www.python.org/) есть такой язык программирования, который позволяет сообщить компьютеру о том, что нужно сделать, чтобы достичь некоего результата. За последнее десятилетие он получил быстрое распространение и сейчас является одним из самых популярных языков программирования в мире. Входной порог для его использования достаточно низок: можно использовать Python для решения своих задач даже если никогда не имели дела с программированием.
...
![](./base/_static/why_py_stackoverflow_1.png)
...
![](./base/_static/why_py_stackoverflow_2.png)
...
![](./base/_static/why_py_stackoverflow_3.png)

## Рейтинг языков программирования

<figure markdown>
  ![](./base/_static/why_py_tiobe_rating.png)
  <figcaption>
...
- ^^ограничения дизайна^^ - Python имеет динамическую типизацию. Это означает, что не нужно объявлять тип переменной при написании кода. Хотя это очень удобно для разработчиков при написании кода, но это может привести к ошибкам при исполнении;
...
Его популярность говорит сама за себя. И эта популярность объясняется тем, что он бесплатный, простой, интерпретируемый, объектно-ориентированный, расширяемый, встраиваемый, переносимый и читабельный.
