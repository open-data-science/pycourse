...
- при помощи инструкции `del` (от английского _«delete»_) можно удалить пару `ключ`: `значение` (_удаление ключа эквивалентно удалению пары `ключ`: `значение`, так как теряем возможность найти то самое `значение`_), в общем виде:
...
!!! question "Можно ли отсортировать словарь по значениям?"

    Да, можно попробовать самостоятельно разобраться с аргументами функции [`sorted()`](https://docs.python.org/3/howto/sorting.html).
...
В англоязычной литературе изменяемые типы называют ^^mutable^^, а неизменяемые – ^^immutable^^, [почитать документацию](https://docs.python.org/3/reference/datamodel.html#objects-values-and-types).
