...
Быстрее всего (как минимум в символах и сразу для всех платформ) рассказать как установить Python, то нужно зайти на официальный сайт $\Rightarrow$ навести курсором на вкладку «Downloads» и выбрать нужную платформу (Linux, macOS, Windows) $\Rightarrow$ скачать релиз с нужной версией (для начала советуем использовать стабильную версию, к примеру, `3.10.6` на момент когда эта лекция пишется)
...
Программное обеспечение, которое позволяет создавать виртуальные окружения в Python можно разделить на те, что входят в стандартную библиотеку Python и не входят в нее. Сделаем краткий обзор доступных инструментов (хороший пост на эту тем есть на [stackoverflow](https://stackoverflow.com/questions/41573587/what-is-the-difference-between-venv-pyvenv-pyenv-virtualenv-virtualenvwrappe)).
...
Для примера рассмотрим работу с `virtualenv`.
...
Установка с помощью `pip` и командной строки
...
!!! info "Флаг/ключ `--system-site-packages`"
...
Если Fedora (или другой похожий дистрибутив, такой как RHEL или CentOS), можно воспользоваться dnf:
...
Если же дистрибутив, основанный на Debian, например, Ubuntu, попробуйте apt:

```bash
sudo apt install git
```

Чтобы воспользоваться дополнительными возможностями, посмотрите инструкцию по установке для нескольких различных разновидностей Unix на сайте Git [git-scm.com/download/linux](https://git-scm.com/download/linux).
...
Тут необходимо установить менеджер пакетов - `brew`. [По ссылке](https://brew.sh/index_ru) найдете инструкцию и более детальное описание, чем является `brew`. Необходимо выполнить в терминале (который доступен как приложение на экране со всеми приложениями – просто начните вводить `term` и поиск подскажет верный вариант) команду:
