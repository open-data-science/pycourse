...
- локальные пространства могут быть вложены друг в друга, в таком случае область уровня `N` (**E** nclosing) содержит «более глубокую» `N + 1`. Поиск имени, вызванного на уровне `N`, начинается с локального namespace этого уровня и продвигается «наружу», то есть на уровни выше;
- глобальные (**G** lobal) — все объекты, определенные на уровне скрипта;
- встроенные (**B** uilt-in) — namespace содержит все встроенные объекты (функции, исключения и т. д.), поиск в нем осуществляется последним.
