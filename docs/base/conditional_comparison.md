...
В данном случае создадутся 2 файла на диске с именем `file1.txt` и `file2.txt`, которые будут содержать строчки «Hello, World 1!» и «Hello, World 2!» соответственно. Но какой еще вид могут иметь контекстные менеджеры в круглых скобках? Вот несколько примеров как их можно использовать в коде:
...
!!! warning "Внимание"
    ...
    В данном разделе лекции, все приведенные примеры содержали только ^^одну^^ функцию/протокол `open()`, так как цель раздела ознакомить слушателя со структурой контекстного менеджера, а не с функционалом протоколов. Позже в курсе будут разбираться протоколы и как их писать с нуля.
...
![](./_static/match_case_1.png){*align="center" width="100%"}
...
Если все еще не верите в удобство этой инструкции, давайте посмотрим на пример, после которого точно не будет сомнений:
